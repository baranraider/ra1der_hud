...
evaHud.CloseAyarMenu = function() {
    ...
    $.post('https://1hud/close-ayar-menu');
};
...
evaHud.updateStatus = function(status){
    $('#yemek-ic').css('width', status.yemek + '%')
    $('#su-ic').css('width', status.su + '%')
};
...
window.onload = function(e) {
    window.addEventListener('message', function(event) {
        switch(event.data.action) {
            ...
            case 'tick':
                ...
                $("#stres-ic").css("width", oxy + '%');

                //$("#stres-ic").css("width", event.data.stres + "%");
                break;
            case 'updateStatus':
    ...
    //     $.post('https://1hud/set-emotechat', JSON.stringify({status: this.checked}));
    // });
    ...
    $('#ghizg-on').on('change',function(){
        $.post('https://1hud/fh4speed', JSON.stringify({status: this.checked}));
        if (this.checked) {
    ...
    $('#pusula-on').on('change',function(){
    	if(this.checked){
            ...
            $.post('https://1hud/set-compass', JSON.stringify({compassOn: true}));
    	}
        else{
            if (pusulaAlt) {
            ...
            $.post('https://1hud/set-compass', JSON.stringify({compassOn: false}));
        }
    });
