...
AddEventHandler("ra1der-hud:car:sikesikehizsabitle", function(toggle, velocity)
    if toggle then
        cruiseIsOn = true
        cruiseSpeed = (tonumber(velocity) / 3.6) / 1.3
    else
        cruiseIsOn = false
    end
end)
...
RegisterNetEvent("ra1der-hud:car:eject-other-player-car-client")
AddEventHandler("ra1der-hud:car:eject-other-player-car-client", function(velocity)
    ejectPlayer()
end)
        ...
        if seatbelt then
            exports['mythic_progbar']:Progress({
                ...
                if not cancelled then
                    exports['mythic_notify']:DoHudText('error', 'Kemer Çıkartıldı!')
            ...
            if IsControlJustReleased(0, 311) and vehicleClass ~= 8 then
                if kemerSayi < 2 and not busy then
                    if not seatbeltDisableExit then 
                        ...
                        if seatbelt then
                            exports['mythic_notify']:DoHudText('success', 'Kemer Takıldı!')
        ...
        if isLoggedIn then
            if inVehicle then
                ...
                if not seatbelt then
                    if vehIsMovingFwd and prevSpeed > 150 and vehAcc then
                        ejectPlayer()
                    else
                ...
                if driverSeat and engineRunning then
                    ...
                    if IsControlJustReleased(0, 246) then
                        if not cruiseIsOn and vehIsMovingFwd then
                            exports['mythic_notify']:DoHudText('success', 'Hız Sabitleyici Aktif!')
                            cruiseSpeed = prevSpeed
                            cruiseIsOn = not cruiseIsOn
                        elseif cruiseIsOn then
                            exports['mythic_notify']:DoHudText('error', 'Hız Sabitleyici İnaktif!')
                            cruiseIsOn = not cruiseIsOn
                        end
                    end
                        ...
                        if vehIsMovingFwd and vehAcc then
                            if prevSpeed*3.6 > triggerSpeed then
                                ...
                                if #seatPlayerId > 0 then
                                    TriggerServerEvent("ra1der-hud:car:eject-other-player-car", seatPlayerId, prevVelocity)
                                end
                                ejectPlayer()
...
function ejectPlayer()
    ESX.TriggerServerCallback('luck:harness', function(isHave)
       if not isHave then
