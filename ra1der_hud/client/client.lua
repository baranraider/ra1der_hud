...
--             -- print(currentWeapon)
--             -- if currentWeapon == 911657153 or currentWeapon == 126349499 then -- Stungun and snowball fix
--             -- else
--             --     if ammo < 2 then
...
--             --         if IsPedInAnyVehicle(player, true) then
--             --             EnableControlAction(0, 69, true) -- Attack
--             --             EnableControlAction(0, 92, true) -- Attack
--             --         end
--             --     end
--             --     if ammo <= 0 then
--             --         -- print('reset')
...
AddEventHandler('reka:hud:client:sendStatus', function(data)
    SendNUIMessage({
        action = "updateStatus",
        st = {yemek = data.hunger, su = data.thirst, stress = data.stress},
    })
end)

RegisterNetEvent('ra1der-hud:big-map')
AddEventHandler('ra1der-hud:big-map', function(bool)
    if Show then
        if bool then
...
exports('setVoiceRange', function(seviye)
    if seviye == 2.5 then
...
RegisterNUICallback('Speedhud', function(data)
    TriggerEvent("fh4speed:hud", data.status)
end)

--[[ local capsAktif = false
RegisterNetEvent('ra1der-hud:client:caps-lock')
AddEventHandler('ra1der-hud:client:caps-lock', function(status)
...
AddEventHandler('esx:playerLoaded', function()
    ...
    ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
        TriggerEvent('skinchanger:cs_karakter', skin)
    end)
--    ESX.ShowNotification("Mevcut şehir vergileri %3", "primary", 15000)
--    TriggerEvent('m3:jail:client:firstSpawn')
--    TriggerEvent('m3:userpeds:spawnPed')
--    TriggerServerEvent('crew:onPlayerLoaded', GetPlayerServerId(PlayerId()))
--    TriggerServerEvent('m3:armor:server:checkArmor')
end)

RegisterCommand('hud', function()
    TriggerEvent('ra1der-hud:open-hud')
...
RegisterNetEvent('ra1der-hud:open-hud')
AddEventHandler('ra1der-hud:open-hud', function()
    if not Show then
        PlayerData = ESX.GetPlayerData()
        SendNUIMessage({action = 'showui'})
        UIStuff()
