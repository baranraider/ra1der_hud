...
AddEventHandler("ra1der_hud:car:sikesikehizsabitle", function(toggle, velocity)
    if toggle then
        cruiseIsOn = true
...
RegisterNetEvent("ra1der_hud:car:eject-other-player-car-client")
AddEventHandler("ra1der_hud:car:eject-other-player-car-client", function(velocity)
    ejectPlayer()
end)
                        ...
                        if vehIsMovingFwd and vehAcc then
                            if prevSpeed*3.6 > triggerSpeed then
                                ...
                                if #seatPlayerId > 0 then
                                    TriggerServerEvent("ra1der_hud:car:eject-other-player-car", seatPlayerId, prevVelocity)
                                end
                                ejectPlayer()
...
function ejectPlayer() 
    ESX.TriggerServerCallback('onemsiz:error', function(isHave)
       if not isHave then
